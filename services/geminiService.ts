...
// This application is designed to load the Gemini API key from an
// environment variable named 'API_KEY'.
//
// To run this application, you must set this environment variable.
// For example, in a terminal session:
// export API_KEY="YOUR_GEMINI_API_KEY"
//
// Alternatively, you can use a .env file with a build tool that supports it.
    ...
    properties: {
        ...
        days: {
            ...
            items: {
                type: Type.OBJECT,
                properties: {
                    day: { type: Type.INTEGER, description: "The day number (1, 2, 3...)." },
                    date: { type: Type.STRING, description: "The full date for this day's plan in YYYY-MM-DD format." },
                    activities: {
                        type: Type.ARRAY,
                        ...
                        items: {
                            ...
                            properties: {
                                ...
                                title: { type: Type.STRING, description: "The name of the activity or meal." },
                                description: { type: Type.STRING, description: "A brief, engaging description of the activity." },
                                travel_time_to_next: { type: Type.STRING, description: "Estimated travel time to the next major activity (e.g., '20 minutes'). Can be null for the last activity.", nullable: true },
                                booking_type: { type: Type.STRING, description: "Type of booking needed. Can be 'hotel', 'restaurant', 'activity', 'taxi', or null.", enum: ['hotel', 'restaurant', 'activity', 'taxi', null], nullable: true },
                                location: { 
                                    type: Type.OBJECT, 
                                    description: "Geographic coordinates (latitude, longitude) of the activity's physical location. MUST be provided for all physical locations. Set to null if not applicable (e.g., 'free time').",
                                    properties: {
...
export const generateItinerary = async (details: TripDetails): Promise<ItineraryPlan> => {
    const { destination, startDate, endDate, personCount, activities, mapPins, budget, searchRadius } = details;

    const budgetPrompt = budget > 0 ? `The total budget for the trip for ${personCount} people is approximately ${budget} INR. Please suggest hotels, activities, and dining options that are mindful of this budget (e.g., provide a mix of options or focus on value-for-money choices).` : 'No specific budget provided.';
    
    const pinsPrompt = mapPins.length > 0 
    ...
    const radiusPrompt = mapPins.length > 0 && searchRadius > 0
