...
const MapComponent: React.FC<MapProps> = ({ destination, pins, onPinsChange, searchRadius }) => {
  if (!MAPS_API_KEY) {
    return (
      <div role="alert" className="relative w-full rounded-lg border p-4 [&>svg]:absolute [&>svg]:text-foreground [&>svg]:left-4 [&>svg]:top-4 [&>svg+div]:translate-y-[-3px] [&:has(svg)]:pl-11 border-destructive/50 text-destructive bg-destructive/10">
        <AlertTriangleIcon className="h-4 w-4" />
        <h5 className="mb-1 font-medium leading-none tracking-tight">API Key Missing</h5>
  ...
  useEffect(() => {
    if (isLoaded && map && destination) {
      ...
      geocoder.geocode({ address: destination, componentRestrictions: { country: 'IN' } }, (results: any, status:any) => {
  ...
  if (loadError) {
    return <div className="flex items-center justify-center h-48 bg-muted/50 rounded-lg text-muted-foreground">Error loading map.</div>;
  }
